...
# MAIN PROGRAM #
################
GetOptions( \%cmdLineArgs, 'contaminants|c=s', 'input_file|f=s',
    'output_dir|o=s', 'table|t', 'log|l=s', 'help|h' )
  or pod2usage();
...
$output_file =
  $outdir . "/" . "new_" . $file_split[-1];    # Create our output file
...
sub process_fasta {
    ...
    open( FO, "> $fsa_file" )
      or printLogMsg( $ERROR,
        "Could not open file $fsa_file for writing. Reason: $!" );
    foreach $contig ( keys %$genome_ref ) {
        ...
        if ( exists( $contaminated_contigs->{$contig} ) ) {
            ...
            my ( $start, $end ) =
              split( /\.\./, $contaminated_contigs->{$contig} );

            $new_seq =
              substr( $new_seq, 0, $start - 1 ) . substr( $new_seq, $end );
...
if ($contig =~ /_55/) { print "---$first\t$second\t$shift_flag\n";}
            ...
            if ( $start > $first && $end < $second ) {
                ...
                my $position = first {
                    ${ $tbl_hash->{$contig} }[$_]->{'locus_tag'} eq
                      $gene->{'locus_tag'}
                }
                0 .. $#{ $tbl_hash->{$contig} }; # Get position of gene in array
                my %new_split = %{$gene};
                ...
                {    # The overlap extends beyond the left
                    $first = $start - $gene->{'shift'};
                    $second = $second - ( $end - $start + 1 ) - $gene->{'shift'};
                    $gene->{'left'} =
                      ( $gene->{'revcom'} ) ? ">" . $first : "<" . $first;
                    $shift_flag += ( $end - $start + 1 );
                } elsif ( $start > $first && $start <= $second )
                {    # The overlap extends beyond the right
                    $second = $start - 1;
                    $gene->{'right'} =
                      ( $gene->{'revcom'} ) ? "<" . $second : ">" . $second;
                    $shift_flag += ( $end - $start + 1 );
                    # applying shift in coordinate position.
                    $first  = $first - $gene->{'shift'};
                    $second = $second - $gene->{'shift'};
                } elsif ( $first > $start && $first > $end )
                {    # The trimming happens entirely on the left side
...
sub readTbl {
    ...
    open( FR, "<$file" )
      or printLogMsg( $ERROR,
        "Could not open file $file for reading. Reason:  $!" );
    ...
    while ( $line = <FR> ) {
        ...
        if ( $line =~ /^>Feature\s+(\S+)/ ) {
            ...
            if (%new_gene) {
                my %passed_gene = %new_gene;
                push @gene, \%passed_gene
                  ;    #if contig has multiple genes, start storing into array
            }
            %new_gene          = ();
            $new_gene{'left'}  = $1;
            $new_gene{'right'} = $2;
...
sub write_tbl {
    ...
    open TBL, ">$tbl_file"
      or printLogMsg( $ERROR, "Cannot open $tbl_file for writing. Reason $!" );
    open( CH, ">$outdir/contamination.tbl" )
      or printLogMsg( $ERROR,
        "Could not open file $outdir/contamination.tbl for writing. Reason: $!"
      );
    ...
    foreach my $contig (@sorted) {
        ...
        foreach my $gene ( @{ $tbl_hash->{$contig} } ) {
            next
              if !( %{$gene} )
              ; # In rare cases where there were no entries for a given feature;
            ...
            print TBL $gene->{'left'}, "\t", $gene->{'right'}, "\t",
              $gene->{'type'}, "\n"
              if ( !defined $gene->{'revcom'} );
            print TBL $gene->{'right'}, "\t", $gene->{'left'}, "\t",
              $gene->{'type'}, "\n"
              if ( defined $gene->{'revcom'} );
            ...
            if ($contam_flag) {
                ...
                print CH "\t\t\tgene\t",      $gene->{'gene'},      "\n"
                  if ( defined $gene->{'gene'} );
                print CH $gene->{'left'}, "\t", $gene->{'right'}, "\t",
                  $gene->{'type'}, "\n"
                  if ( !defined $gene->{'revcom'} );
                print CH $gene->{'right'}, "\t", $gene->{'left'}, "\t",
                  $gene->{'type'}, "\n"
                  if ( defined $gene->{'revcom'} );
            }
            ...
            delete $gene->{'type'};
            delete $gene->{'gene'};
            delete $gene->{'locus_tag'};

            foreach my $feat ( keys %{$gene} ) {
                next
                  if ( $feat eq "revcom" )
                  ;    # skip hash keys that was created to aid in processing
...
sub parseContaminants {
    ...
    open( FR, "< $filename" )
      or printLogMsg( $ERROR,
        "Could not open file $filename for reading. Reason: $!" );
    while ( $line = <FR> ) {
        chomp($line);
        next if ( $line =~ /^\s*$/ );
...
sub checkCmdLineArgs {
    ...
    foreach my $option (@required) {
        if ( !defined( $cmdLineArgs{$option} ) ) {
            printLogMsg( $ERROR,
                "ERROR! : Required option $option not passed" );
        }
    }
    if ( exists( $cmdLineArgs{'log'} ) ) {
        open( $logfh, "> $cmdLineArgs{'log'}" )
          or die
          "Could not open $cmdLineArgs{'log'} file for writing.Reason : $!\n";
    }
